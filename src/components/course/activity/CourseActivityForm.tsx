...
// Fallback list used if reftbl returns empty
const FALLBACK_ACTIVITY_TYPES = ['Assignment', 'Task', 'Notification'];
const SCOPE_OPTIONS: CourseScope[]       = ['course', 'school'];
const GRADING_OPTIONS: GradingTarget[]   = ['individual', 'class'];
const STATUS_OPTIONS: ActivityStatus[]   = ['open', 'in-progress', 'in-review'];

export default function CourseActivityForm({ navigation, route }: Props) {
  const { mode, item, prefilledCourse } = route.params;
  ...
  // ── Form state ────────────────────────────────────────────────────────────
  const [activityType,    setActivityType]    = useState(item?.activityType     ?? 'Assignment');
  const [scope,           setScope]           = useState<CourseScope>(item?.scope         ?? 'course');
  ...
  const [coordinator,     setCoordinator]     = useState(item?.coordinator      ?? '');
  const [reviewer,        setReviewer]        = useState(item?.reviewer         ?? '');
  const [gradingTarget,   setGradingTarget]   = useState<GradingTarget>(item?.gradingTarget ?? 'individual');
  ...
  const [ratingNote,      setRatingNote]      = useState(item?.ratingNote ?? '');
  ...
  const [activityTypeOptions, setActivityTypeOptions] = useState<string[]>(FALLBACK_ACTIVITY_TYPES);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [cats, types] = await Promise.all([
        getRefOptions('assignmentCatRef'),
        getRefOptions('assignmentTyperef'),
      ]);
      if (!cancelled) {
        setCategoryOptions(cats);
        setActivityTypeOptions(types.length > 0 ? types : FALLBACK_ACTIVITY_TYPES);
  ...
  // ── Teacher options (assignee / assignor) ─────────────────────────────────
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled) {
        setTeacherOptions(rows.map((t) => t.email ?? t.name ?? '').filter(Boolean).sort());
  ...
  // ── Student coordinator options — reactive to selected course ────────────
  const [studentOptions,   setStudentOptions]   = useState<string[]>([]);
  const [loadingStudents,  setLoadingStudents]  = useState(false);

  useEffect(() => {
    if (!course.trim()) { setStudentOptions([]); return; }
    let cancelled = false;
    setLoadingStudents(true);
    studentRepository.findByCourse(course).then((rows) => {
      if (!cancelled) {
        setStudentOptions(
          rows
            .filter((s) => s.status === 'active' && (s.fullName || s.regNumber))
            .map((s) => s.fullName ?? s.regNumber ?? '')
            .filter(Boolean)
            .sort(),
        );
  ...
  // ── Course options with class-teacher auto-fill ───────────────────────────
  ...
  const handleCourseChange = useCallback((label: string) => {
    ...
    setCourse(newCourse);
    // Auto-fill assignee with class teacher
    if (idx >= 0) {
  ...
  const validate = useCallback((): boolean => {
    ...
    if (isAddEdit) {
      if (!activityType.trim())     errs.activityType = 'Activity type is required';
      ...
      if (scope === 'course' && !course.trim()) errs.course = 'Course is required when scope is "course"';
    ...
    if (isReview && gradingTarget === 'class' && !isOverdue) {
  ...
  }, [isAddEdit, isReview, activityType, title, startDate, assignor, assignee, scope, course, endDate, isNotification, reviewer, gradingTarget, ratingRaw, isOverdue]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const entry: CourseActivityModel = {
        ...
        coordinator:  isAddEdit ? (coordinator.trim() || undefined) : item?.coordinator,
        reviewer:     isAddEdit ? (reviewer.trim()    || undefined) : item?.reviewer,
        gradingTarget: isAddEdit ? gradingTarget                    : item?.gradingTarget,
        status:       nextStatus,
        ...
        ratingNote:    isReview ? (ratingNote.trim() || undefined) : item?.ratingNote,
        closedBy,
        closedAt:      isReview ? new Date().toISOString() : item?.closedAt,
        revision:      nextRevision,
        norm_rating:   computeActivityNormRating(
          (isAddEdit ? activityType : item?.activityType) as any,
          resolvedRating,
        ),
      };

      await courseActivityRepository.save(entry);
      syncSheet(SHEETS.COURSE_ACTIVITY).catch(() => {});
      const successMsg =
  ...
  }, [validate, isEdit, isSubmit, isReview, isAddEdit, item, activityType, scope, course, category, title, description, startDate, endDate, assignor, assignee, coordinator, reviewer, gradingTarget, status, submissionNote, ratingRaw, ratingNote, navigation, snackbar, isOverdue]); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Delete ────────────────────────────────────────────────────────────────
  const confirmDelete = useCallback(() => {
    setSaving(true);
    courseActivityRepository.delete(item!.id)
      .then(() => {
        syncSheet(SHEETS.COURSE_ACTIVITY).catch(() => {});
        navigation.goBack();
      })
  ...
  // ── Labels ────────────────────────────────────────────────────────────────
  const headerLabel =
    mode === 'add'    ? 'New Course Activity' :
    mode === 'edit'   ? 'Edit Course Activity' :
      ...
      <ScrollView contentContainerStyle={KStyles.formScroll} keyboardShouldPersistTaps="handled">
          ...
          <>
            ...
            <Field label="Scope" required>
              <SingleSelectDropdown
                ...
                onChange={(v) => setScope(v as CourseScope)}
            ...
            <Field label="Grading Target" required>
              <SingleSelectDropdown
                selected={gradingTarget}
                options={GRADING_OPTIONS}
                onChange={(v) => setGradingTarget(v as GradingTarget)}
                placeholder="Select grading target…"
            ...
            <Field label="Assignee (Coordinator)" required>
              <SingleSelectDropdown
                selected={assignee}
            ...
            <Field label="Student Coordinator (optional)">
              {studentOptions.length > 0 ? (
                <SingleSelectDropdown
                  selected={coordinator}
                  options={studentOptions}
                  onChange={setCoordinator}
                  placeholder="Select student coordinator…"
                ...
                <InputField
                  ...
                  placeholder={course.trim() ? 'No students found for this course…' : 'Select a course first…'}
                  editable={!!course.trim()}
                />
              )}
            </Field>
        ...
        {isReview && (
          <>
            ...
            {item?.gradingTarget === 'individual' ? (
              <View style={styles.infoBox}>
                <Ionicons name="information-circle-outline" size={18} color={Colors.primary} style={{ marginRight: 8 }} />
                <Text style={styles.infoBoxText}>
                  This activity uses individual grading. Please rate each student via their
                  linked Student Activity records.
                </Text>
              </View>
            ) : isOverdue ? (
            ...
            {item?.gradingTarget !== 'individual' && (
              <Field label="Rating Note">
                <InputField
                  ...
                  placeholder="Reviewer note…" multiline editable
                />
              </Field>
            )}
          </>
