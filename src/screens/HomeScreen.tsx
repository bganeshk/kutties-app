...
import { ensureSheets, backfillNormRating } from '../sync/sync.service';
...
export default function HomeScreen({ navigation }: Props) {
  ...
  // Ensure sheets exist in the workbook, backfill norm_rating, then sync dashboard on first mount
  useEffect(() => {
    if (!synced.current) {
      synced.current = true;
      ensureSheets()
        .catch(() => {})
        .then(() => backfillNormRating())
        .catch(() => {})
        .finally(() => sync());
    }
  }, []);
