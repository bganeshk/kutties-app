...
import { courseActivityRepository, teacherRepository } from '../db/repositories';
...
type Props = NativeStackScreenProps<HomeStackParamList, 'CourseActivityDetails'>;
...
function TypeBadge({ type }: { type?: string }) {
  if (!type) return null;
  const c = TYPE_COLORS[type] ?? { bg: '#F0F4FF', text: '#1565C0' };
  return (
...
function ScopeTag({ scope, gradingTarget }: { scope?: string; gradingTarget?: string }) {
  return (
    <View style={styles.heroChips}>
      {scope ? (
        <View style={styles.scopeTag}>
          <Ionicons
            name={scope === 'school' ? 'business-outline' : 'school-outline'}
            size={11}
            color="#fff"
            style={{ marginRight: 4 }}
          />
      ...
      {gradingTarget ? (
        <View style={[styles.scopeTag, styles.gradingTag]}>
          <Ionicons
            name={gradingTarget === 'class' ? 'ribbon-outline' : 'people-outline'}
            size={11}
            color="#fff"
            style={{ marginRight: 4 }}
          />
          <Text style={styles.scopeTagText}>{gradingTarget} grading</Text>
        </View>
      ) : null}
    </View>
  );
...
function StarRow({ rating }: { rating?: number }) {
  ...
  return (
    <View style={styles.starRow}>
      {[1, 2, 3, 4, 5].map((s) => (
        <Ionicons key={s} name={s <= rating ? 'star' : 'star-outline'} size={20} color="#FDD835" />
      ))}
...
export default function CourseActivityDetailsScreen({ navigation, route }: Props) {
  const [item,          setItem]          = useState<CourseActivityModel>(route.params.item);
  ...
  useFocusEffect(
    useCallback(() => {
      courseActivityRepository.findById(route.params.item.id).then((fresh) => {
  ...
  const canSubmit      = item.status === 'in-progress' && (isAssignment || isTask);
  const canReview      = item.status === 'in-review';

  const handleDelete = useCallback(() => {
    courseActivityRepository.delete(item.id).then(() => {
  ...
  return (
    <SafeAreaView style={KStyles.detailsRoot}>
      ...
      <View style={KStyles.header}>
        ...
        <Text style={KStyles.headerTitle} numberOfLines={1}>Course Activity</Text>
        <View style={KStyles.headerActions}>
          <TouchableOpacity
            style={KStyles.headerIcon}
            onPress={() => navigation.navigate('CourseActivityForm', { mode: 'edit', item })}
      ...
      <ScrollView contentContainerStyle={KStyles.detailsScroll}>
        ...
        <View style={KStyles.detailsHeroCard}>
          <View style={[styles.heroAvatar, typeStyle && { backgroundColor: typeStyle.bg }]}>
            <Ionicons
              name={
                ...
                isTask       ? 'checkmark-circle'  : 'notifications'
              }
              size={36}
          ...
          <ScopeTag scope={item.scope} gradingTarget={item.gradingTarget} />
        </View>
        ...
        <View style={KStyles.detailsCard}>
          ...
          />
          {item.coordinator ? (
            <InfoRow icon="person-outline" label="Student Coordinator" value={item.coordinator} iconBg={PRIMARY} />
          ) : null}
          {item.reviewer ? (
        ...
        <View style={KStyles.detailsCard}>
          <InfoRow
            icon={item.gradingTarget === 'class' ? 'ribbon-outline' : 'people-outline'}
            label="Grading Target"
            value={item.gradingTarget === 'class' ? 'Class (single rating)' : 'Individual (per student)'}
            iconBg={PRIMARY}
          />
          {item.gradingTarget === 'class' && item.status === 'closed' && (
          ...
          {item.ratingNote ? (
            <InfoRow icon="chatbubble-outline" label="Rating Note" value={item.ratingNote} />
          ) : null}
        </View>

        {/* ── Submission ─────────────────────────────────────────────────── */}
        {(item.submissionAttachments?.length || item.submissionNote) ? (
          <>
        ...
        {item.closedBy ? (
          <>
            ...
            <View style={KStyles.detailsCard}>
              ...
              {item.closedAt ? (
                <InfoRow icon="time-outline" label="Closed At" value={item.closedAt} />
              ) : null}
            </View>
          </>
        ) : null}
        ...
        <View style={styles.actionRow}>
          {canSubmit && (
            <TouchableOpacity
              ...
              activeOpacity={0.85}
              onPress={() => navigation.navigate('CourseActivityForm', { mode: 'submit', item })}
            >
          ...
          {canReview && (
            <TouchableOpacity
              ...
              activeOpacity={0.85}
              onPress={() => navigation.navigate('CourseActivityForm', { mode: 'review', item })}
            >
...
const styles = StyleSheet.create({
  ...
  gradingTag: { backgroundColor: PRIMARY },
  scopeTagText: { fontSize: 11, fontWeight: '700', color: '#fff' },
