...
import { studentActivityRepository, studentRepository, teacherRepository } from '../db/repositories';
...
type Props = NativeStackScreenProps<HomeStackParamList, 'StudentActivityDetails'>;
...
export default function StudentActivityDetailsScreen({ navigation, route }: Props) {
  const [item,          setItem]          = useState<StudentActivityModel>(route.params.item);
  ...
  useFocusEffect(
    useCallback(() => {
      studentActivityRepository.findById(route.params.item.id).then((fresh) => {
        if (fresh) setItem(fresh);
      });
    }, [route.params.item.id]),
  );

  // ── Resolve student & assignor ────────────────────────────────────────────
  React.useEffect(() => {
    if (!item.assignee) return;
    studentRepository.findAll().then((rows) => {
      const match = rows.find((s) => s.regNumber?.toLowerCase() === item.assignee?.toLowerCase());
      setStudent(match);
    });
  }, [item.assignee]);
  ...
  React.useEffect(() => {
    ...
    teacherRepository.findAll().then((rows) => {
      const match = rows.find(
        (t) => (t.email ?? t.name ?? '').toLowerCase() === item.assignor?.toLowerCase(),
      );
  ...
  const handleDelete = useCallback(() => {
    studentActivityRepository.delete(item.id).then(() => {
      ...
      <View style={KStyles.header}>
        ...
        <View style={KStyles.headerActions}>
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('StudentActivityForm', { mode: 'edit', item })}
      ...
      <ScrollView contentContainerStyle={KStyles.detailsScroll}>
        ...
        <View style={KStyles.detailsCard}>
          ...
          <InfoRow
            icon="person-outline"
            label="Assignee"
            value={studentName ?? item.assignee}
            iconBg={PRIMARY}
        ...
        <View style={styles.actionRow}>
          {canSubmit && (
            <TouchableOpacity
              ...
              activeOpacity={0.85}
              onPress={() => navigation.navigate('StudentActivityForm', { mode: 'submit', item })}
            >
          ...
          {canReview && (
            <TouchableOpacity
              ...
              activeOpacity={0.85}
              onPress={() => navigation.navigate('StudentActivityForm', { mode: 'review', item })}
            >
